...
const Ventas = () => {
  ...
  const handleBarcodeScanned = async (barcode) => {
    try {
      ...
      // Check if it's a QR code from our system (JSON format)
      try {
        const qrData = JSON.parse(barcode);
        if (qrData.type === 'FERRETERIA_PRODUCT' && qrData.id) {
          // It's a QR code generated by our system
          product = products.find(p => p.id === qrData.id);
          if (product) {
            addToCart(product);
            toast.success(`Producto agregado (QR): ${product.nombre}`);
            return;
          }
        }
      } catch {
        // Not a JSON QR, continue with barcode search
      }
      
      // Search product by barcode
      product = products.find(p => p.codigo_barras === barcode);
