  ...
  try {
    ...
    headers.forEach((header, index) => {
      ...
      if (lower.includes("nombre") || lower.includes("name")) {
        columnMap.nombre = index.toString();
      } else if (
        lower.includes("código") ||
        lower.includes("codigo") ||
        lower.includes("barras") ||
        lower.includes("barcode")
      ) {
        columnMap.codigoBarras = index.toString();
      } else if (
        lower.includes("minorista") ||
        lower.includes("retail") ||
        lower.includes("precio minorista")
      ) {
        columnMap.precioMinorista = index.toString();
      } else if (
        lower.includes("mayorista") ||
        lower.includes("wholesale") ||
        lower.includes("precio mayorista")
      ) {
        columnMap.precioMayorista = index.toString();
      }
    });

    if (
      !columnMap.nombre ||
      columnMap.precioMinorista === undefined ||
      columnMap.precioMayorista === undefined
    ) {
      return {
        success: false,
        error:
          "El archivo debe contener las columnas: Nombre, Precio Minorista, Precio Mayorista",
      };
    }
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      try {
        const nombre = row[columnMap.nombre as any]
          ? String(row[columnMap.nombre as any]).trim()
          : "";
        if (!nombre) {
        ...
        const codigoBarras =
          columnMap.codigoBarras !== undefined && row[columnMap.codigoBarras]
            ? String(row[columnMap.codigoBarras]).trim()
            : undefined;

        const precioMinorista = parseFloat(
          String(row[columnMap.precioMinorista] || 0),
        );
        const precioMayorista = parseFloat(
          String(row[columnMap.precioMayorista] || 0),
        );
